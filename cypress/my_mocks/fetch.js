...
const fetch = (url) =>
  Promise.resolve({
    ...
    json: () => {
      ...
      if (
        url === 'https://www.themealdb.com/api/json/v1/1/search.php?s=xablau'
      ) {
      ...
      if (
        ...
        return Promise.resolve(firstDrink);
      }

      if (
        url === 'https://www.themealdb.com/api/json/v1/1/lookup.php?i=52977'
      ) {
        return Promise.resolve(firstMeal);
      }
      ...
      if (url === 'https://www.thecocktaildb.com/api/json/v1/1/search.php?f=g') {
        return Promise.resolve(firstDrink);
      }
